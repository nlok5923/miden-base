use super::{compiler::TransactionComplier, AccountId, ModuleAst, ProgramAst};
...
fn test_load_account() {
    let mut tx_compiler = TransactionComplier::default();
    let account_id =
...
fn test_compile_valid_note_script() {
    ...
    let target_account_proc = Some(account_code.procedures().to_vec());
    ...
    for (note_script_src, expected) in test_cases {
        ...
        let result = tx_compiler.compile_note_script(note_script_ast, &target_account_proc);
        match expected {
