...
fn chain_mmr_memory_assertions<A: AdviceProvider>(
    ...
    assert_eq!(
        process.get_mem_value(0, CHAIN_MMR_NUM_LEAVES_PTR).unwrap()[0],
    ...
    for (i, peak) in inputs.block_chain().mmr().accumulator().peaks.iter().enumerate() {
        ...
        assert_eq!(process.get_mem_value(0, CHAIN_MMR_PEAKS_PTR + i).unwrap(), Word::from(peak));
    }
}
...
fn account_data_memory_assertions<A: AdviceProvider>(
    ...
    assert_eq!(
        process.get_mem_value(0, ACCT_ID_AND_NONCE_PTR).unwrap(),
...
fn consumed_notes_memory_assertions<A: AdviceProvider>(
    ...
    assert_eq!(
        process.get_mem_value(0, CONSUMED_NOTE_SECTION_OFFSET).unwrap()[0],
    ...
    for (note, note_idx) in inputs.consumed_notes().notes().iter().zip(0u32..) {
        ...
        for (asset, asset_idx) in note.vault().iter().cloned().zip(0u32..) {
            ...
            assert_eq!(
                process
                    .get_mem_value(0, consumed_note_data_ptr(note_idx) + 6 + asset_idx)
                    .unwrap(),
                word
            );
        }
